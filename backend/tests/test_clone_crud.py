...
from app.services.persona import build_clone_profile
...
class CloneCrudApiTests(unittest.TestCase):
    ...
    def test_user_can_list_update_use_and_delete_only_own_clones(self):
        ...
        bob.post("/api/v1/auth/register", json={"username": "Bob", "password": "s3cret"})
        ...
        listed = alice.get("/api/v1/clones").json()
        bob_listed = bob.get("/api/v1/clones").json()
        ...
        renamed = alice.put(
            f"/api/v1/clones/{clone_id}", json={"targetName": "Dad Renamed"}
        ).json()
        self.assertEqual(renamed["clone"]["cloneId"], clone_id)
        self.assertEqual(renamed["clone"]["name"], "Dad Renamed")

        forbidden = bob.get(f"/api/v1/clones/{clone_id}")
        self.assertEqual(forbidden.status_code, 404)

        deleted = alice.delete(f"/api/v1/clones/{clone_id}").json()
        self.assertTrue(deleted["ok"])
        self.assertEqual(alice.get("/api/v1/clones").json()["clones"], [])
        missing = alice.get(f"/api/v1/clones/{clone_id}")
