...
from cyberclone.api.conversions import (
...
class WebApiIntegrationTests(unittest.TestCase):
    ...
    def _patch_paths(self, root: Path) -> None:
        from cyberclone.api import deps, routers
        from cyberclone.api.routers import voice as voice_router
    ...
    def test_get_voice_status_returns_persisted_training_status(self):
        ...
        response = client.get(f"/api/clones/{profile.clone_id}/voice/status")
    ...
    def test_chat_correction_message_is_persisted_and_acknowledged(self):
        ...
        response = client.post(
            f"/api/clones/{profile.clone_id}/chat",
