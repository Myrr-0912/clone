...
class AuthWebApiTests(unittest.TestCase):
    ...
    def test_clone_api_requires_login(self):
        client = self._client()
        response = client.post("/api/clones", json={"targetName": "Dad", "chatText": "Dad: hi"})
    ...
    def test_admin_endpoint_requires_admin_and_returns_vector_resources(self):
        ...
        alice.post("/api/clones", json={"targetName": "Dad", "chatText": "Dad: latte"})

        forbidden = alice.get("/api/admin/vector-dbs")
        self.assertEqual(forbidden.status_code, 403)

        auth.register("Root", "admin-pass", role="admin")
        admin = self._client()
        admin.post("/api/auth/login", json={"username": "root", "password": "admin-pass"})

        response = admin.get("/api/admin/vector-dbs")
